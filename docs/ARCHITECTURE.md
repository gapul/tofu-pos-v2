...
| `database/` | （未使用、drift 設定は `lib/data/datasources/local/` 配下） |
...
| feature | 状態 |
|---|---|
| `dev_console/` | ✅ 仮UI、全機能の手動検証用 |
| `regi/` | domain/checkout_flow_usecase.dart のみ。UI は Figma 待ち |
| `kitchen/`, `calling/`, `settings/`, `startup/` | 空、UI は Figma 待ち |
